...
/* Tests to verify that macros have not broken functionality */
void run_tests() {
	vector v1 = {1.0, 2.0, 3.0};
	vector v2 = {4.0, 5.0, 6.0};

	/* Test vectorSub */
	vector r1 = vectorSub(&v1, &v2);
	assert(r1.x == -3.0 && r1.y == -3.0 && r1.z == -3.0);

	/* Test vectorDot */
	float r2 = vectorDot(&v1, &v2);
	assert(r2 == 32.0);

	/* Test vectorScale */
	vector r3 = vectorScale(2.0, &v2);
	assert(r3.x == 8.0 && r3.y == 10.0 && r3.z == 12.0);

	/* Test vectorAdd */
	vector r4 = vectorAdd(&v1, &v2);
	assert(r4.x == 5.0 && r4.y == 7.0 && r4.z == 9.0);
}
...
int main(int argc, char *argv[]){
	...
	saveppm("/tmp/d5chau_image.ppm", img, WIDTH, HEIGHT);
